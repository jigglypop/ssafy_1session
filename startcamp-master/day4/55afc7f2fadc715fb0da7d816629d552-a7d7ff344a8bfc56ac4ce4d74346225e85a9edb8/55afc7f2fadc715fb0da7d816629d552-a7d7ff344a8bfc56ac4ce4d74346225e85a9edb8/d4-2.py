...
for x,y in ssafy['language']['python']['frameworks'].items():
    print(f'{x}는 {y}이다.')
...
print(random.choice(ssafy['classes']['dj']['groups']['D']))


# .get() 없으면 오류를 띄우지 않는다
"""
ex)
dictionary.get('cat')-->아무것도 안뜸
dictionary.get('cat', '등록되지 않음')을 넣으면
'등록되지 않음'으로 출력
